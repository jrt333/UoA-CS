...
def test_final_bounding_box():
    ...
    assert bounding_box_len == 1, "Too few or too many detected bounding boxes!"
    
    target_box = [525, 332, 824, 575]
    student_box = bounding_box_list[0]
    
    IoU = bbox_iou(target_box, student_box)
    
    assert IoU >= 0.9, f"Bounding box(es) too small or too large!"
